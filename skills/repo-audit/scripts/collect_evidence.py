...
VIEW_FIELDS = (
    "name,nameWithOwner,description,repositoryTopics,homepageUrl,isPrivate,"
...
def readme_shape(text: str) -> dict:
    ...
    return {
        ...
        "code_block_first_60_lines": any(
            ln.lstrip().startswith(("```", "~~~")) for ln in lines[:60]
        ),
    }
...
def main() -> int:
    ...
    evidence: dict = {
        "repo": repo,
        "collected_via": "local+api" if local else "api",
    ...
    og = gh_json(
        [
            ...
            "graphql",
            "-f",
            f'query=query{{repository(owner:"{owner}",name:"{name}"){{usesCustomOpenGraphImage}}}}',
            "--jq",
    ...
    ci_run = gh_json(
        ["run", "list", "--repo", repo, "--limit", "1", "--json", "conclusion,workflowName"]
    )
    ...
    def exists(path: str):
        ...
        if local:
            ...
            return any(
                (base / name_only).is_file() and (base / name_only).stat().st_size > 0
    ...
    evidence["files"]["issue_forms"] = (
        None
        if names is None
        else len(
            [n for n in names if n.endswith((".yml", ".yaml")) and not n.startswith("config.")]
        )
    )
    evidence["files"]["issue_template_config"] = (
        None if names is None else any(n.startswith("config.") for n in names)
    )
    wf = listing(".github/workflows")
    evidence["files"]["workflows"] = (
        None if wf is None else len([n for n in wf if n.endswith((".yml", ".yaml"))])
    )
    ...
    prot["classic_protection"] = (
        True if classic.returncode == 0 else (False if "404" in (classic.stderr or "") else None)
    )
    sec = gh_json(["api", f"repos/{repo}", "--jq", ".security_and_analysis"])
    prot["secret_scanning"] = ((sec or {}).get("secret_scanning") or {}).get("status")
    prot["push_protection"] = ((sec or {}).get("secret_scanning_push_protection") or {}).get(
        "status"
    )
    alerts = gh(["api", f"repos/{repo}/vulnerability-alerts"])
    prot["dependabot_alerts"] = (
        True if alerts.returncode == 0 else (False if "404" in (alerts.stderr or "") else None)
    )
