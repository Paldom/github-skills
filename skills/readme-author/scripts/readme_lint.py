...
BADGE_URL_RE = re.compile(
    r"(shields\.io|badge\.svg|badgen\.net|/badge/|codecov\.io|"
    r"coveralls\.io|img\.shields|/actions/workflows/[^)]*badge)",
    re.I,
)
PLACEHOLDER_ANGLE_RE = re.compile(
    r"<[a-z][a-z0-9-]*(?:-[a-z0-9]+)+>|<(?:placeholder|your-[a-z-]+)>", re.I
)
...
HYPE_RE = re.compile(
    ...
    r"cutting[- ]edge|next[- ]generation|ultimate)\b",
    re.I,
)
...
def main() -> int:
    ...
    if len(raw) > 500 * 1024:
        errors.append(
            f"file is {len(raw) // 1024} KiB - GitHub truncates rendered READMEs above 500 KiB"
        )
    ...
    if not h1s:
        ...
        errors.append(
            f"{len(h1s)} H1 headings (lines {', '.join(str(h[0]) for h in h1s)}) - use exactly one"
        )
    ...
    badge_count = sum(
        1 for ln in lines[:15] for m in IMAGE_RE.finditer(ln) if BADGE_URL_RE.search(m.group(1))
    )
    if badge_count > 5:
    ...
    if not any(FENCE_RE.match(ln) for ln in lines[:60]):
        warns.append(
            "no code block in the first 60 lines - the quick start should be copy-pasteable early"
        )
    ...
    if h1s:
        ...
        if intro_words > 160:
            warns.append(
                f"intro block before the first ## is ~{intro_words} words - aim under ~120"
            )
    ...
    for i, ln in enumerate(lines, 1):
        ...
        if BAD_LINK_TEXT_RE.search(ln):
            errors.append(
                f"line {i}: non-descriptive link text ('click here'/'here') - name the destination"
            )
        if HYPE_RE.search(ln):
            warns.append(
                f"line {i}: hype word ({HYPE_RE.search(ln).group(1)!r}) - show, don't tell"
            )
        ...
        if PLACEHOLDER_ANGLE_RE.search(prose):
            warns.append(
                f"line {i}: template placeholder {PLACEHOLDER_ANGLE_RE.search(prose).group(0)} "
    ...
    if len(lines) > 400:
        ...
        if not has_toc:
            warns.append(
                f"README is {len(lines)} lines with no TOC - add one or move depth into docs/"
            )
